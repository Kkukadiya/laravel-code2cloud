...
const WhatsAppButton = ({ 
  ...
  return (
    <div className="fixed bottom-20 left-6 lg:bottom-6 lg:left-8 z-50 flex flex-col items-start gap-3">
      {/* Tooltip */}
      {showTooltip && (
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="group relative flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-all duration-300 hover:scale-110"
          style={{
            background: 'linear-gradient(135deg, #006BFF 0%, #0041A8 100%)',
          }}
          aria-label="Book on Calendly"
        >
          <CalendarDays className="w-7 h-7 text-white transition-transform duration-300 group-hover:scale-110" />
        </button>
      </div>
    </div>
  );
